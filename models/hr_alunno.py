...
class scuola_alunno_hr(models.Model):
    ...
    _description = "Alunno da impiegati"
    ...
    attendance_ids = fields.One2many('scuola.presenza', 'alunno_id', help='lista di presenza per alunno')
    last_attendance_id = fields.Many2one('scuola.presenza', compute='_compute_last_attendance_id', store=True)
    attendance_state = fields.Selection(string="Presenza Status", compute='_compute_attendance_state', selection=[('checked_out', "Checked out"), ('checked_in', "Checked in")])
    manual_attendance = fields.Boolean(string='Presenza Manuale', compute='_compute_manual_attendance', inverse='_inverse_manual_attendance',
